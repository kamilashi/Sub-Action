...
namespace Library
{
    public static class SmoothingFuncitons
    {
        ...
        public static float ApproachReferenceLinear(float input, float reference, float riseSpeed, float fallSpeed)
        {
            float direction = System.Math.Sign(reference - input);
            float output;
            if (direction > 0)
            {
                output = input + direction * riseSpeed;
                output = System.Math.Min(output, reference);
            }
            else
            {
                output = input - direction * fallSpeed;
                output = System.Math.Max(output, reference);
            }
